...
public class TableChangeTrigger extends TriggerAdapter {
    ...
    public static final String TRIGGER_INSERT_STATEMENT = "INSERT INTO AUDIT_TRAIL(table_name, id, timestamp, change_type, notified) VALUES (?, ?, CURRENT_TIMESTAMP(), ?, ?);";

    @Override
    public void fire(Connection connection, ResultSet oldRow, ResultSet newRow) throws SQLException {
        log.debug("Database trigger [{}] fired on table [{}]", this.triggerName, this.tableName);
        ...
        preparedStatement.setString(2, newRow.getString("ID"));
